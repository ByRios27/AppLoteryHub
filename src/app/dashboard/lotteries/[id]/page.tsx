...
export default function LotterySalePage() {
    ...
    const { sales, setSales, lotteries, specialPlays, updateSale } = useStateContext();
    ...
    const handleSaleSubmit = (values: StandardSaleFormFields | MultiPickSaleFormFields) => {
        ...
        if (editingSale) {
            ...
            const newSale = { ...saleData, id: `S${Date.now()}` };
    ...
    const handleEditSale = (sale: Sale) => {
        ...
        if (sale.specialPlayId && playType === 'multi_pick') {
            multiPickForm.reset({
                customerName: sale.customerName,
                customerPhone: sale.customerPhone,
                picks: sale.tickets[0].ticketNumber.split(' - '),
            });
        } else {
            standardSaleForm.reset({
                customerName: sale.customerName,
                customerPhone: sale.customerPhone,
    ...
    return (
        <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="grid w-full grid-cols-2"><TabsTrigger value="sell">{editingSale ? "Editando Venta" : "Vender"}</TabsTrigger><TabsTrigger value="sales">Ventas Realizadas</TabsTrigger></TabsList>
                        ...
                        <CardContent>
                            {playType === 'multi_pick' ? (
                                ...
                                <div className="flex justify-end pt-4 gap-2">
                                    {editingSale && <Button type="button" variant="outline" onClick={resetFormsAndState}><X className="mr-2 h-4 w-4"/>Cancelar Edición</Button>}
                                    <Button type="submit" className="w-full sm:w-auto" disabled={isMultiPickButtonDisabled}><PlusCircle className="mr-2 h-4 w-4" />{editingSale ? 'Guardar Cambios' : 'Completar Jugada'}</Button>
                                </div>
                                ...
                                <Form {...standardSaleForm}><form onSubmit={standardSaleForm.handleSubmit(handleSaleSubmit as any)} className="space-y-6">
                                    ...
                                    <div className="flex justify-end gap-2 pt-4">
                                        {editingSale && <Button type="button" variant="outline" onClick={resetFormsAndState}><X className="mr-2 h-4 w-4"/>Cancelar Edición</Button>}
                                        <Button type="submit" className="w-full sm:w-auto"><PlusCircle className="mr-2 h-4 w-4" />{editingSale ? 'Guardar Cambios' : 'Completar Venta'}</Button>
                                    </div>
                                </form></Form>
                            )}
                        </CardContent>
                ...
                <TabsContent value="sales" className="pt-6">
                     <Card>
                        ...
                        <CardContent><Table>
                            <TableHeader><TableRow><TableHead>Números/Jugada</TableHead><TableHead>Sorteos</TableHead><TableHead>Cliente/Fecha</TableHead><TableHead className="text-right">Total</TableHead><TableHead className="text-right">Acciones</TableHead></TableRow></TableHeader>
                            <TableBody>{salesForItem.length > 0 ? (salesForItem.map((sale) => (<TableRow key={sale.id}><TableCell className="font-mono text-base font-bold">{sale.tickets.map(t => t.ticketNumber).join(', ')}</TableCell><TableCell className="text-xs">{sale.draws.map(d => (lotteries.find(l=>l.id===d.lotteryId)?.name||"") + " - " + d.drawTime).join(', ')}</TableCell><TableCell>{sale.customerName || "-"}<p className="text-xs text-muted-foreground font-mono">{new Date(sale.soldAt).toLocaleString()}</p></TableCell><TableCell className="text-right font-semibold">${sale.totalCost.toFixed(2)}</TableCell><TableCell className="text-right"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="h-8 w-8 p-0"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => handleViewReceipt(sale)}><Eye className="mr-2 h-4 w-4"/>Ver Comprobante</DropdownMenuItem><DropdownMenuItem onClick={() => handleEditSale(sale)}><Edit className="mr-2 h-4 w-4"/>Editar</DropdownMenuItem><DropdownMenuItem onClick={() => alert('Share not implemented')}><Share2 className="mr-2 h-4 w-4"/>Compartir</DropdownMenuItem><DropdownMenuItem onClick={() => setSaleToDelete(sale.id)} className="text-destructive"><Trash2 className="mr-2 h-4 w-4"/>Borrar</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>))) : (<TableRow><TableCell colSpan={5} className="text-center h-24">No se han registrado ventas.</TableCell></TableRow>)}</TableBody>
                        </Table></CardContent>
                    </Card>
                </TabsContent>
            </Tabs>
